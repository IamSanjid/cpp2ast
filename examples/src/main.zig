...
pub fn main() !void {
    ...
    if (args.next()) |arg| {
        if (std.mem.eql(u8, arg, "index_zig")) {
            ...
            var src = try index_zig.bindings(allocator, inc_header_path);
            defer src.deinit();
        ...
        if (std.mem.eql(u8, arg, "index_glue")) {
            ...
            var header = try index_glue.generate_header(allocator, inc_header_path);
            defer header.deinit();
            ...
            var source = try index_glue.generate_source(allocator, inc_header_path, header_name);
            defer source.deinit();
